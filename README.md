...
**默认是空库**：只有两个账户（匿名访客落到的 demo，和审核用的 reviewer），
没有做市方、没有挂单、没有联系人、没有余额。要拿真实流程做验收时就该是
这样——写死评分和成交记录的假商家混在自己挂的单里，根本分不清哪个是真的。

要那套演示数据（10 家做市方 + 10 条挂单 + 联系人 + 余额 + 额度）：

```bash
ATARA_SEED=true make fresh
```
...
准入审核那条循环是**另一条**，固定每秒：它只是把库里一行的布尔位翻过来，
不碰链。跟链上扫描共用一个节奏的话，链一改成每分钟，交完材料的人就要盯着
「审核中」干等一分钟——而那是任何人打开演示第一眼看到的东西。
